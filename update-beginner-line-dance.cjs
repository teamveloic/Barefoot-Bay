/**
 * Quick script to update BEGINNER LINE DANCE event
...
// EVENT ID TO UPDATE
const EVENT_ID = 344; // BEGINNER LINE DANCE event
...
/**
 * Main function to update the BEGINNER LINE DANCE event
 */
async function updateBeginnerLineDanceEvent() {
  try {
    console.log(`Starting personalization for BEGINNER LINE DANCE (ID: ${EVENT_ID})`);
    ...
    if (eventImages.length > 0) {
      ...
      console.log(`Successfully updated BEGINNER LINE DANCE with ${eventImages.length} images`);
    } else {
      console.log(`No images found for BEGINNER LINE DANCE`);
    }
  } catch (err) {
...
// Run the update
updateBeginnerLineDanceEvent().catch(console.error);