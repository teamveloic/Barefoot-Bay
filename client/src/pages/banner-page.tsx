...
export default function BannerPage() {
  ...
  const handleContentRestoration = useCallback((event: CustomEvent) => {
    console.log("🎯 Received banner-content-restored event:", event.detail);
  ...
  useEffect(() => {
    ...
    function processHash() {
      ...
      // Map slide numbers to the old section name format for backward compatibility
      const sectionMap = {
        1: "golf",
        2: "tennis", 
        3: "pools",
      ...
      // Use the mapped section if available, otherwise use the slide format directly
      const section = slideIndex <= Object.keys(sectionMap).length ? 
        sectionMap[slideIndex as keyof typeof sectionMap] : 
        `slide${slideIndex}`;
    ...
    const handleHashChange = () => {
      ...
      // Use the mapped section if available, otherwise use the slide format directly
      const newSection = slideIndex <= Object.keys(sectionMap).length ? 
        sectionMap[slideIndex as keyof typeof sectionMap] : 
        `slide${slideIndex}`;
  ...
  // Always use dash format for consistency (amenities-golf, not amenities#golf)
    ...
    switch (currentSection) {
      ...
      default:
        if (currentSection.startsWith("slide")) {
          return `Banner Slide ${currentSection.replace("slide", "")}`;
        }
        return "Barefoot Bay Banner Slide";
    }
  };
