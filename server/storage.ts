...
export class DatabaseStorage implements IStorage {
  ...
  /* First publishListing method removed */
  ...
  async deleteMessage(id: number): Promise<boolean> {
    ...
    try {
      // First, mark any orphaned replies as read to prevent notification inconsistencies
      await this.cleanupOrphanedReplies(id);
  ...
  /**
   * Clean up orphaned replies when a parent message is deleted
   * This prevents unread notification badge inconsistencies
   */
  private async cleanupOrphanedReplies(parentMessageId: number): Promise<void> {
    try {
      console.log(`Cleaning up orphaned replies for parent message ${parentMessageId}`);
      
      // Mark all replies to this message as read to prevent orphaned unread notifications
      const cleanupResult = await pool.query(`
        UPDATE message_recipients 
        SET read_at = NOW()
        WHERE message_id IN (
          SELECT id FROM messages WHERE in_reply_to = $1
        )
        AND read_at IS NULL
      `, [parentMessageId]);
      
      if (cleanupResult.rowCount > 0) {
        console.log(`Marked ${cleanupResult.rowCount} orphaned replies as read for parent message ${parentMessageId}`);
      }
    } catch (error) {
      console.error(`Error cleaning up orphaned replies for message ${parentMessageId}:`, error);
      // Don't throw - this shouldn't block the main deletion
    }
  }
  ...
  async getUnreadMessageCount(userId: number): Promise<number> {
    ...
    try {
      // Using an improved query that joins with the messages table 
      // to ensure we only count unread messages that actually exist
      ...
      console.log(`User ID: ${userId} has ${unreadCount} unread messages (validated count)`);
      return unreadCount;
    } catch (error) {
