/**
 * Quick script to update AQUATIC AEROBICS event
...
// EVENT ID TO UPDATE
const EVENT_ID = 342; // AQUATIC AEROBICS event
...
/**
 * Main function to update the AQUATIC AEROBICS event
 */
async function updateAquaticAerobicsEvent() {
  try {
    console.log(`Starting personalization for AQUATIC AEROBICS (ID: ${EVENT_ID})`);
    ...
    if (eventImages.length > 0) {
      ...
      console.log(`Successfully updated AQUATIC AEROBICS with ${eventImages.length} images`);
    } else {
      console.log(`No images found for AQUATIC AEROBICS`);
    }
  } catch (err) {
...
// Run the update
updateAquaticAerobicsEvent().catch(console.error);