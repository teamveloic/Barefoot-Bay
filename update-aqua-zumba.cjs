/**
 * Quick script to update a single event (AQUA ZUMBA)
...
// EVENT ID TO UPDATE
const EVENT_ID = 343; // AQUA ZUMBA event
...
/**
 * Main function to update the AQUA ZUMBA event
 */
async function updateAquaZumbaEvent() {
  try {
    console.log(`Starting personalization for AQUA ZUMBA (ID: ${EVENT_ID})`);
    ...
    if (eventImages.length > 0) {
      ...
      console.log(`Successfully updated AQUA ZUMBA with ${eventImages.length} images`);
    } else {
      console.log(`No images found for AQUA ZUMBA`);
    }
  } catch (err) {
...
// Run the update
updateAquaZumbaEvent().catch(console.error);