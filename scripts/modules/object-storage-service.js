...
// server/object-storage-service.ts
var object_storage_service_exports = {};
__export(object_storage_service_exports, {
  BUCKETS: () => BUCKETS,
  objectStorageService: () => objectStorageService
});
module.exports = __toCommonJS(object_storage_service_exports);
var import_object_storage = require("@replit/object-storage");
...
var BUCKETS = {
  DEFAULT: "DEFAULT",
  // For avatars, banner slides, and site-wide icons
  CALENDAR: "CALENDAR",
  // For calendar events
  FORUM: "FORUM",
  // For forum media
  VENDORS: "VENDORS",
  // For vendor-related media
  SALE: "SALE",
  // For real estate/for-sale items
  COMMUNITY: "COMMUNITY"
  // For community-related media
};
var MEDIA_TYPE_TO_BUCKET = {
  "avatar": BUCKETS.DEFAULT,
  "banner": BUCKETS.DEFAULT,
  "banner-slides": BUCKETS.DEFAULT,
...
var ObjectStorageService = class {
  client;
  baseUrl = "https://object-storage.replit.app";
  constructor() {
    this.client = new import_object_storage.Client();
  }
  /**
   ...
   */
  async uploadFile(filePath, directory, filename) {
    try {
      ...
      const actualFilename = filename || import_path.default.basename(filePath);
      const bucket = this.getBucketForMediaType(directory);
      const storageKey = directory ? `${directory}/${actualFilename}` : actualFilename;
      const contentType = import_mime_types.default.lookup(actualFilename) || "application/octet-stream";
      console.log(`[ObjectStorage] Uploading file to ${bucket}/${storageKey}...`);
      const result = await this.client.uploadFromFilename(storageKey, filePath, {
        contentType,
        bucketName: bucket
      });
      if (!result.ok) {
      ...
      return `${this.baseUrl}/${bucket}/${storageKey}`;
    } catch (error) {
   ...
   * @returns URL to the uploaded file
   */
  async uploadData(data, mediaType, filename, contentType) {
    try {
      if (!data) {
        console.error(`No data provided for upload of ${filename} in ${mediaType}`);
        return `/media-placeholder/${mediaType}.png`;
      }
      const fileBuffer = typeof data === "string" ? Buffer.from(data) : data;
      if (fileBuffer.length === 0) {
        console.error(`Empty buffer provided for upload of ${filename} in ${mediaType}`);
        return `/media-placeholder/${mediaType}.png`;
      }
      ...
      const storageKey = `${safeMediaType}/${safeFilename}`;
      ...
      import_fs.default.writeFileSync(tempFilePath, fileBuffer);
      let attempts = 0;
      const maxAttempts = 3;
      let lastError;
      ...
      return `/media-placeholder/${safeMediaType}.png`;
    } catch (error) {
      console.error(`[ObjectStorage] Error uploading data for ${filename}:`, error);
      return `/media-placeholder/${mediaType || "default"}.png`;
    }
  }
  /**
  ...
  async fileExists(key) {
    try {
      const result = await this.client.list({ prefix: key, maxResults: 1 });
      if (result.ok && result.value.length > 0) {
        return result.value.some((obj) => obj.name === key);
      }
      return false;
    } catch (error) {
   ...
   */
  getBucketForMediaType(mediaType) {
   ...
   */
  async migrateFromPostgreSQL(dbId, table, column, mediaType, filenameColumn) {
    try {
      ...
      const contentType = import_mime_types.default.lookup(filename) || "application/octet-stream";
